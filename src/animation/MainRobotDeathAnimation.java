 ...
 * @author Utente
 */
public class MainRobotDeathAnimation extends AbstractAnimation{
    private MainRobotSprites mRSprites;
    private int posX;
    private int xShift;
    
    
    public MainRobotDeathAnimation(AnimationPanel animPanel){
        super(null,null, animPanel);
        mRSprites = new MainRobotSprites();
        this.animFrameArray = mRSprites.getMainRobotDeathSprites();
        //this.animShiftMatrix = mRSprites.getMainRobotDeathShiftArray();
    };
    ...
    public void showCurrentFrame(Graphics g, int x, int y){
        ...
        xShift = this.animShiftMatrix[this.currentFrameIndex][0];
        int yShift = this.animShiftMatrix[this.currentFrameIndex][1];
        posX = x;
        int moveX = (posX + xShift - mRSprites.standardWidth);
        int moveY = (y + yShift - mRSprites.standardHeight);
        
        if(this.getMaxX()>(1950- mRSprites.standardWidth)){
            moveX= posX;
