 ...
 * @author Utente
 */
public class MainRobotJumpAnimation extends AbstractAnimation{
    ...
    public MainRobotJumpAnimation(AnimationPanel animPanel){
        super(null,null, animPanel);
        mRSprites = new MainRobotSprites();
        this.animFrameArray = mRSprites.getMainRobotJumpSprites();
        //this.animShiftMatrix = mRSprites.getMainRobotJumpShiftArray();
    };
    ...
    public void showCurrentFrame(Graphics g, int x, int y){
        ...
        int moveX = (x + xShift - mRSprites.standardWidth);
        int moveY = (y + yShift - mRSprites.standardHeight);
        if(this.getMaxX()>(1920- mRSprites.standardWidth)){
            moveX= posX;
